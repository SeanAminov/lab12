...
public class FamilyTree
{
        ...
        TreeNode getNodeWithName(String targetName)
        {
            ...
            for (TreeNode child: children)
            {
            	...
            	if (child.children.size() != 0) {
            		TreeNode returner = child.getNodeWithName(targetName);
            		if (returner == null) {
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("Colon Index is Negative");
	...
	//	System.out.println(childrenString);
		...
		//	System.out.println("Root:" + root);
			parentNode = root.getNodeWithName(parent); //.parent; //?????  There's a method in Node that searches for a named node. 
			//??? If the parent node wasn't found, there must have been something wrong in the 
		//		data file. Throw an exception.
			if (parentNode == null) {
				throw new TreeException("Parent node wasn't found");
			}
		}
	...
	//	TreeNode test 
		for (int i = 0; i < childrenArray.length; i++) {
			TreeNode test = new TreeNode(childrenArray[i]);
			parentNode.addChild(test);
		}
	}
